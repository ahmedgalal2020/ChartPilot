...
export async function getCurrentUser(): Promise<User | null> {
  ...
  return getOrCreateProfile(session.user.id, session.user.email || '', session.user.user_metadata?.name);
}
...
export async function signInWithPassword(email: string, password: string): Promise<User> {
  ...
  return getOrCreateProfile(data.user.id, data.user.email || email, data.user.user_metadata?.name);
}
...
export async function signUpWithPassword(email: string, password: string, name: string): Promise<User | null> {
  ...
  return getOrCreateProfile(data.user.id, data.user.email || email, name);
}
